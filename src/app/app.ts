          ...
          @if (store.phase() === 0) {
            ...
            <div class="hidden lg:flex items-center text-sm font-medium text-zinc-400 mr-2 border-r border-zinc-200 pr-6">
              <div class="flex items-center space-x-3">
                ...
                <button (click)="goToPhase(2)" [disabled]="(!store.rawMarkdown() && !store.rawPdf()) || store.isBusy()" class="flex items-center hover:text-indigo-600 transition-colors cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:text-zinc-400" [class.text-indigo-600]="store.phase() >= 2">
                  <span class="w-5 h-5 text-xs rounded-full border-2 flex items-center justify-center mr-1.5"
...
export class App {
  ...
  goToPhase(phase: number) {
    ...
    } else if (phase === 2 && (this.store.rawMarkdown() || this.store.rawPdf())) {
