    ...
    <div class="py-8">
      <div class="max-w-5xl mx-auto lg:px-8 px-4 flex items-center justify-between mb-8">
        <div>
          ...
          <p class="text-zinc-500 mt-1">Sử dụng mô hình AI mạnh để phân tích nội dung, giúp xây dựng bảng đại từ nhân xưng hoàn chỉnh, nhằm đảm bảo nhất quán khi dịch & phù hợp hơn với văn hóa người Việt. <strong class="font-semibold text-zinc-700">Có thể cần thiết với dạng sách kiểu tự sự (kể chuyện). Các loại sách thuần túy khoa học có thể không cần thiết (click vào "Bỏ qua phần này").</strong></p>
...
export class PronounSetup {
  ...
  async startGeneration() {
    ...
    const chapters = this.store.chapters();
    let chunks: import('../../core/db').PronounChunk[] = [];
    
    if (chapters && chapters.length > 0 && chapters[0].originalPdfBase64) {
  ...
  async processPronounTask() {
    ...
    try {
      ...
      if (allCompleted) {
        ...
        const fullPdf = this.store.rawPdf();
        let result = '';
        if (fullPdf) {
            let binary = '';
            const len = fullPdf.byteLength;
