...
export class GlossarySetup {
  ...
  async startGeneration() {
    ...
    const chapters = this.store.chapters();
    let chunks: import('../../core/db').GlossaryChunk[] = [];
      ...
      for (let i = 0; i < chunksToProcess.length; i += maxConcurrent) {
        ...
        const promises = batch.map(async chunk => {
          try {
            ...
            const result = await this.gemini.generateGlossaryRaw(inputData, isPdf, task.model, this.store.bookTitle(), this.store.author());
            chunk.result = result;
