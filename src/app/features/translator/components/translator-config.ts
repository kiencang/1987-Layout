  ...
  template: `
    <div class="bg-white rounded-xl shadow-sm border border-zinc-200 p-6 mb-8 flex flex-col gap-8 relative">
        ...
        <div class="flex-1">
          ...
          <div class="flex flex-col space-y-2">
            <label class="flex items-center space-x-3 transition-opacity"
              ...
              <span class="text-sm text-zinc-700 font-medium tracking-tight">Khoa học nói chung</span>
            </label>
            <label class="flex items-center space-x-3 transition-opacity"
                    [class.cursor-pointer]="!store.isTranslatingAny()"
                    [class.cursor-not-allowed]="store.isTranslatingAny()"
              ...
              <input type="radio" name="translationStyle" value="social_science" 
                ...
                class="w-4 h-4 text-indigo-600 border-zinc-300 focus:ring-indigo-500 disabled:cursor-not-allowed">
              <span class="text-sm text-zinc-700 font-medium tracking-tight">Khoa học xã hội</span>
            </label>
            <label class="flex items-center space-x-3 transition-opacity"
                    [class.cursor-pointer]="!store.isTranslatingAny()"
                    [class.cursor-not-allowed]="store.isTranslatingAny()"
              ...
              <input type="radio" name="translationStyle" value="specialized_math" 
                ...
                class="w-4 h-4 text-indigo-600 border-zinc-300 focus:ring-indigo-500 disabled:cursor-not-allowed">
              <span class="text-sm text-zinc-700 font-medium tracking-tight">Toán chuyên ngành</span>
            </label>
          </div>
        </div>
      ...
      <div class="pt-6 border-t border-zinc-200 flex items-center justify-between">
        <label class="flex items-center space-x-3 transition-opacity cursor-pointer" [class.cursor-not-allowed]="store.isTranslatingAny() || store.chapters().length <= 1" [class.opacity-50]="store.isTranslatingAny() || store.chapters().length <= 1">
          <input type="checkbox" 
            [checked]="store.chapters().length > 1 && store.config().generateSummary !== false"
            (change)="toggleGenerateSummary($event)"
            [disabled]="store.isTranslatingAny() || store.chapters().length <= 1"
            class="w-4 h-4 text-amber-600 rounded border-zinc-300 focus:ring-amber-500 disabled:cursor-not-allowed"
            [class.cursor-pointer]="!store.isTranslatingAny() && store.chapters().length > 1">
          <span class="text-zinc-700 font-medium tracking-tight">Tạo bản tóm tắt cho khối/chương dịch kế tiếp</span>
        </label>
        <div class="text-xs text-zinc-500 max-w-lg">
          @if (store.chapters().length <= 1) {
            <span class="text-amber-600 font-medium">[Đã tự động Tắt] - Cuốn sách hiện tại chỉ có 1 khối/chương duy nhất nên không cần tạo tóm tắt cho khối tiếp theo.</span>
          } @else {
            <span>[Mặc định Bật] - Tự động tóm tắt nội dung sau khi dịch xong một khối để đưa bối cảnh vào khối dịch kế tiếp. Hữu ích khi các chương/khối dịch là một phần của cuốn sách tổng thể. Nếu các chương/khối hoàn toàn độc lập, ví dụ như các truyện ngắn riêng biệt trong một cuốn sách lớn thì nên tắt tùy chọn này.</span>
          }
        </div>
      </div>
