...
export type ProjectMeta = Omit<Project, 'chapters' | 'rawMarkdown' | 'rawPdf' | 'pronounTable' | 'glossaryTable' | 'pdfTask' | 'pronounTask' | 'glossaryTask' | 'pronounVersions' | 'glossaryVersions' | 'images'>;
export type ProjectAsset = Pick<Project, 'rawMarkdown' | 'rawPdf' | 'pronounTable' | 'glossaryTable' | 'pdfTask' | 'pronounTask' | 'glossaryTask' | 'pronounVersions' | 'glossaryVersions' | 'images'>;
...
export class DbService {
  ...
  async getProject(id: string): Promise<Project | undefined> {
    try {
      ...
      const [
        rawMdReq, rawPdfReq, pronounsReq, glossaryReq, pVersReq, gVersReq, pdfReq, pTaskReq, gTaskReq, imagesReq
      ] = await Promise.all([
        assetStore.get(`${id}_rawMarkdown`),
        assetStore.get(`${id}_rawPdf`),
      ...
      if (rawPdfReq) asset.rawPdf = rawPdfReq.data as typeof asset.rawPdf;
  ...
  async saveProject(project: Project): Promise<void> {
    try {
      ...
      if (project.rawPdf) {
         await assetStore.put({ id: `${project.id}_rawPdf`, data: project.rawPdf } as { id: string; data: unknown });
      }
  ...
  async deleteProject(id: string): Promise<void> {
    try {
      ...
      await assetStore.delete(`${id}_rawPdf`);
