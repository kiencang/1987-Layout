...
export class BookStore {
  ...
  readonly rawPdf = signal<Uint8Array | undefined>(undefined);
  readonly pdfPageCount = signal<number | undefined>(undefined);
    ...
    effect(() => {
      ...
      untracked(() => {
        if (isPlatformBrowser(this.platformId)) {
          ...
          if (rawPdf) {
            this.db.saveProjectAsset(projectId, 'rawPdf', rawPdf);
          } else {
            this.db.saveProjectAsset(projectId, 'rawPdf', undefined);
          }
          if (images) {
  ...
  async loadProject(id: string) {
    ...
    if (proj) {
      ...
      this.rawPdf.set(proj.rawPdf);
      this.pdfPageCount.set(proj.pdfPageCount);
      ...
      const isSingleChapter = adjustedChapters.length <= 1;
      this.config.set({
        translationStyle: 'general_science',
        ...proj.config,
        generateSummary: isSingleChapter ? false : (proj.config?.generateSummary !== false)
      });
  ...
  setPdf(pdf: Uint8Array, name: string, pageCount?: number) {
    const safePdf = new Uint8Array(pdf.byteLength);
    safePdf.set(pdf);
    this.rawPdf.set(safePdf);
  ...
  setChapters(chs: Chapter[]) {
    ...
    this.phase.set(3);
    if (chs.length <= 1) {
      this.config.update(c => ({ ...c, generateSummary: false }));
    } else {
      this.config.update(c => ({ ...c, generateSummary: true }));
    }
    if (isPlatformBrowser(this.platformId)) {
